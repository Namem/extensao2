...
confirmando que o modelo aprendeu o fundo como feature discriminativa, não as lesões.
...
O Ceres Diagnóstico se diferencia por: (a)~validação em 3~datasets de campo
independentes em três continentes; (b)~\textit{pipeline} IoT completo
(MQTT + REST + Flutter); (c)~documentação de 5~experimentos com resultados negativos
reprodutíveis.
...
\textit{representative\_dataset} \cite{jacob2018}. Elimina o quantization loss
observado no Exp.~A ($-$30,5~pp sem calibração). \textbf{Pós-processamento:}
temperature scaling ($T=0{,}25$) aplicado ao vetor de logits antes do softmax
em todos os caminhos de inferência (ESP32-S3, Android on-device e Django),
calibrando a confiança reportada ao usuário.
...
Tomato-Village \cite{girase2024} --- 217~imagens (Rajastão, Índia, 2022);
...
System}) do dispositivo antes de cada inferência e a inclui no POST de diagnóstico. Cache local gerenciado pelo
Drift (SQLite) com \texttt{SyncService} que reenvia automaticamente diagnósticos
pendentes ao restaurar conectividade (\textit{offline-first}).
...
QoS~0/1/2, projetado para redes instáveis e MCUs com recursos limitados. O HTTP
...
escala (scale e
zero-point) por tensor.

O Exp.~B, com calibração sobre 50~batches reais do val set, eliminou o quantization
loss: INT8 (98,13\%) superou o FP32 do Exp.~A (92,5\%) em 5,6~pp. Esse resultado
indica que a calibração adequada preserva não apenas a acurácia, mas também pode
melhorá-la ao forçar o modelo a representar as distribuições reais dos ativadores.
...
alocados (39\%), deixando margem para modelos maiores. A estimativa do Edge Impulse
(1.365~ms) foi superada em 2$\times$, indicando que simuladores tendem a
subestimar a eficiência real do Xtensa~LX7 em operações INT8.
...
fundo cinza), forçando o modelo a focar nas features diagnósticas da lesão.
...
\begin{enumerate}
  ...
  \item \textbf{Experimento Edge vs.\ Cloud com hardware real:} latência medida
        empiricamente, não estimada.
\end{enumerate}
