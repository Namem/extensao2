...
experiments document the impact of INT8 calibration (+36~pp), synthetic background
augmentation (ineffective), and real-data fine-tuning (+10~pp). The final model
achieves 98.43\% on PlantVillage and 18--30\% on three independent field datasets,
quantifying the persistent lab-field gap.
\end{abstract}

\keywords{TinyML, MobileNetV2, ESP32-S3, Detecção de Doenças em Plantas,
Aprendizado de Máquina Embarcado, Gap Laboratório-Campo}
...
(+36~pp), augmentation sintética (ineficaz) e fine-tuning real (+10~pp). O modelo
final atinge 98,43\% no PlantVillage e 18--30\% em três datasets de campo,
quantificando o gap laboratório-campo.
\end{resumo}

\palavraschave{TinyML, MobileNetV2, ESP32-S3, Detecção de Doenças em Plantas,
Aprendizado de Máquina Embarcado, Gap Laboratório-Campo}
...
qualidade de serviço (QoS~0, 1 e~2), tornando-o superior ao HTTP REST (cabeçalho
$\approx$800~bytes, sem QoS nativo) para comunicação ESP32~→~servidor em ambientes
rurais.
...
EarlyStopping($p=8$) + ReduceLROnPlateau($f=0{,}5$). Melhor val\_acc: 97,79\%
(época~28).

\textbf{Quantização INT8 calibrada:} 50~batches do val set como
\textit{representative\_dataset} \cite{jacob2018}, eliminando o quantization loss
observado sem calibração ($-$30,5~pp). \textbf{Pós-processamento:} temperature
scaling ($T=0{,}25$) nos logits antes do softmax em todos os caminhos de inferência.
...
Tomato-Village \cite{girase2024} --- 217~imagens (Rajastão, Índia);
...
medida no ESP32-S3 e a análise do gap laboratório-campo nos três datasets de campo.
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Métrica} & \textbf{Exp.~A FP32} & \textbf{Exp.~A INT8} & \textbf{Exp.~B INT8} \\
\midrule
Acurácia val set  & 92,5\% & 62,0\% & 97,79\% \\
Acurácia test set & ---    & ---    & \textbf{98,13\%} \\
Macro F1          & 0,92   & 0,62   & 0,977 \\
Tamanho modelo    & 1.637~KB & 547~KB & \textbf{639~KB} \\
\bottomrule
\end{tabular}
\end{table}
...
as com menos amostras no val set, evidenciando que a representatividade do
\textit{dataset} de calibração determina diretamente a qualidade dos fatores de
escala por tensor.

O Exp.~B, com calibração sobre 50~batches reais, eliminou o quantization loss:
INT8 (98,13\%) superou o FP32 do Exp.~A (92,5\%) em 5,6~pp.
...
\begin{enumerate}
  \item \textbf{Pipeline reproduzível:} PlantVillage $\to$ 88.949~imgs $\to$
        ...
        (\url{https://github.com/Namem/extensao2}).
  \item \textbf{Quantificação da calibração INT8:} $+$36~pp com
        \textit{representative\_dataset} vs.\ quantização automática.
